...
class LineDataExtract : DataExtract {
    ...
    override func dataSetConfig(_ dataSet: ChartDataSet, config: JSON) {
        ...
        if config["circleRadius"].number != nil {
            lineDataSet.circleRadius = CGFloat(truncating: config["circleRadius"].numberValue)
        }
        ...
        if config["drawCubicIntensity"].number != nil {
            lineDataSet.cubicIntensity = CGFloat(truncating: config["drawCubicIntensity"].numberValue);
        }
        ...
        if config["dashedLine"].exists() {
            ...
            if dashedLine["lineLength"].number != nil {
                lineLength = CGFloat(truncating: dashedLine["lineLength"].numberValue)
            }
            if dashedLine["spaceLength"].number != nil {
                spaceLength = CGFloat(truncating: dashedLine["spaceLength"].numberValue)
            }
            if dashedLine["phase"].number != nil {
                phase = CGFloat(truncating: dashedLine["phase"].numberValue)
            }
